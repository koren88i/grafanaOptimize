...
**5. Weighted composite scoring.** No OSS tool produces a dashboard health score. Framework: weight structural issues (panel count, missing units = low), query issues (regex abuse, missing filters = medium-high), performance issues (cardinality, range windows = high), and operational issues (cache miss rates, execution times = critical) into a 0–100 score. Tunable per-org thresholds. Score calculation: `100 − Σ(severity_weight × violation_count)`, clamped to [0,100]. Severity weights: Critical = 15, High = 10, Medium = 5, Low = 2.
